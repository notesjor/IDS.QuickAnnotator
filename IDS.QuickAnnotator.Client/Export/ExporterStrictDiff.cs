...
namespace IDS.QuickAnnotator.Client.Export
{
  public class ExporterStrictDiff : AbstractSimpleExporter
  {
    ...
    protected override void PreProcessing(string path)
    {
      ...
      _res.Add("DocumentID\tLength\tAnnotator (A)\tAnnotator (B)\tLayer\tNur (A)\tNur (B)\tMatches\tUnterschiede (A+B)\tAgreement (A+B in %)");
    }
    ...
    private void DiffQuick(string[] annoA, string[] annoB, out int length, out int plusA, out int plusB, out int common, out int diff)
    {
      length = 0;
      plusA = 0;
      plusB = 0;
