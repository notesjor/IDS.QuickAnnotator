...
namespace IDS.QuickAnnotator.Client
{
  public partial class DashboardForm : AbstractForm
  {
    ...
    public DashboardForm()
    {
      ...
      foreach (var d in _anno.AvailableDocumentIds.Where(x => !x.Contains("_")))
      {
    ...
    {
      var lk = radio_lk_del_5.IsChecked ? "" : (chk_lk_6.IsChecked ? "?" : "") + (radio_lk_1.IsChecked ? "1" : radio_lk_2.IsChecked ? "2" : radio_lk_3.IsChecked ? "3" : "");
      ...
      var del = (lk + ab + re + ma + ko + se).Length > 0;
